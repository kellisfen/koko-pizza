...
export default function MenuItemCard({ item }: MenuItemCardProps) {
  ...
  // Проверяем есть ли варианты размеров
  const hasSizes = !!pizzaSizes[item.id];
    ...
    <>
      <div className="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition">
        ...
        <div className="p-4">
          ...
          {hasSizes && (
            <div className="text-xs text-[#FF6B35] mb-2 font-medium">
              Выберите размер
            </div>
          )}
      ...
      {showSizes && (
        <SizeSelector item={item} onClose={() => setShowSizes(false)} />
      )}
    </>
  );
}
